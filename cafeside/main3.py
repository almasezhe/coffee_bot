...
# Если нужна вторая конфигурация, можно раскомментировать:
...
# Создаем роутер и включаем его в диспетчер
router = Router()
dp.include_router(router)

# === ОБРАБОТЧИК ОШИБОК (AIROGRAM 3.0) ===
@router.errors()
async def handle_errors(update: types.Update, exception: Exception, data: dict) -> bool:
    if isinstance(exception, TelegramNetworkError):
        logger.warning(f"Проблема с сетью: {exception}. Повторная попытка через 5 секунд.")
        await asyncio.sleep(5)
        return True  # Говорим, что ошибка обработана, будем повторять
...
async def render_menu(message_or_callback, page: int = 0):
    ...
    for item in menu:
        ...
        buttons.append([
            ...
            # Если нужен delete-кнопка, раскомментируй:
            # InlineKeyboardButton(text="Удалить", callback_data=f"delete_confirm_{item['menu_id']}_{page}")
        ])
    # Если нужна кнопка добавления напитка, можно добавить:
    # buttons.append([InlineKeyboardButton(text="Добавить напиток", callback_data="add")])
...
message_cache_lock = asyncio.Lock()  # Блокировка для синхронизации доступа к кэшу
...
async def monitor_order_status():
    ...
    while True:
        try:
            async with message_cache_lock:
                ...
                for order in canceled_orders:
                    ...
                    logger.info(
                        f"Hash check for message_id={message_id}: cached={current_hash}, new={new_hash}"
                    )
        ...
        await asyncio.sleep(5)  # Интервал ожидания
...
async def show_orders_callback(callback_query: types.CallbackQuery):
    ...
    for order in orders:
        ...
        message = await callback_query.message.answer(
            ...
            f"Напиток: {order['coffee_name']}\n"
            f"{order['take_out']}\n"
            f"Комментарий: {order.get('details','Нет комментария')}\n",
            reply_markup=keyboard,
        )
...
async def handle_cafe_cancel_order(callback_query: types.CallbackQuery):
    try:
...
async def complete_order(callback_query: types.CallbackQuery):
    try:
        ...
        await callback_query.message.edit_text(
            ...
            f"Комментарий: {order_details.get('details','Нет комментария')}\n"
            f"Статус был обновлён на: готов",
            reply_markup=keyboard
        )
        await callback_query.answer("Статус заказа обновлён.")
    except Exception as e:
        logger.error(f"Ошибка в обработке 'done^': {e}")
